...
### Added
- New `annotate` command: write typed changelog blocks (`changelog:added`, `changelog:fixed`, …) in your PR descriptions and `release-it-preset annotate` turns them into curated `[Unreleased]` entries — verbatim, idempotent, merged PRs only, and it never writes anything when a lookup fails. (#65) ([d42604f](https://github.com/oorabona/release-it-preset/commit/d42604f))
- `doctor` gains two release-pipeline checks: publish workflow freshness (workflows generated by `init --with-workflows` are compared against the shipped template; hand-written workflows are never judged) and npm provenance readiness (verifies `permissions: id-token: write` is effective on the publishing job itself when `NPM_PUBLISH=true`, modeling GitHub Actions job-level permission overrides). (#64) ([261e4bf](https://github.com/oorabona/release-it-preset/commit/261e4bf))
- `doctor` now checks workspace dependency range coherence: internal package ranges are compared against the current workspace versions, with advisory warnings for stale or unevaluable ranges. `workspace:*`/`^`/`~` references pass automatically and single-package repositories are unaffected. (#62) ([bd40852](https://github.com/oorabona/release-it-preset/commit/bd40852))

### Changed
- Composing with `@release-it-plugins/workspaces` is now exercised end-to-end in CI: the documented monorepo configuration (release-it 19, `publish: false` + `skipChecks: true`) is asserted against a real temp workspace, and the release-it 20 peer incompatibility is locked by a test. (#63) ([7a02626](https://github.com/oorabona/release-it-preset/commit/7a02626))
- Our own workflows and the generated `init --with-workflows` template follow least-privilege OIDC: `id-token: write` lives on the publishing jobs only (#64) ([261e4bf](https://github.com/oorabona/release-it-preset/commit/261e4bf))
- README slimmed for onboarding: `init` hoisted into Quick Start, long-form usage extracted to `docs/USAGE.md` (#54) ([a15cf05](https://github.com/oorabona/release-it-preset/commit/a15cf05))
- Development tooling refreshed: dev-dependency group (#59), codecov-action 4→7 (#60), github-script 7→9 (#56), @types/node 25.9.2, biome config migrated to the 2.4.16 schema with all lint diagnostics cleared ([54fa495](https://github.com/oorabona/release-it-preset/commit/54fa495))

### Fixed
- `doctor` reports an advisory warning instead of throwing when the changelog vanishes between its existence check and its read ([13e451b](https://github.com/oorabona/release-it-preset/commit/13e451b))

### Security
- The summary job of PR validation no longer interpolates branch-derived outputs into `github-script` as quoted strings (`toJSON()` everywhere), closing a script-injection path in a job holding `pull-requests: write` (#65) ([d42604f](https://github.com/oorabona/release-it-preset/commit/d42604f))
- The vulnerable `undici <6.24.0` range pulled by the release-it 19 development tree is overridden to `>=6.24.0 <7` (development scope only, nothing ships) ([75f1a3a](https://github.com/oorabona/release-it-preset/commit/75f1a3a))
