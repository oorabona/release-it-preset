...
| `doctor` | `doctor` | Structured diagnostic: 4-section checklist + `--json` output, exit 1 on BLOCKED. Configuration checks include: `release-it peer dependency` (`semver` decides whether the installed version satisfies the `peerDependencies.release-it` range; FAIL if absent, not valid semver, or out of range; WARN when the declared range is not a valid semver range and nothing can be concluded), `release-it major version` (evaluates whether the latest published release-it version satisfies the declared peer range; skipped on network failure, and skipped when the declared peer range is not a valid semver range, which the peer check already reports), `Workspace dependency ranges` (workspace projects only: internal package dependency ranges checked against current workspace versions; advisory WARN on stale or unevaluable ranges, never FAIL; omitted on single-package repos), `publish workflow freshness` (marker-bearing workflows generated by `init --with-workflows` are compared to the shipped template; hand-written workflows are not judged), `npm provenance readiness` (when `NPM_PUBLISH=true`, checks local GitHub Actions workflow files for `permissions: id-token: write`; zero network calls; advisory WARN only), and `SLSA attestation availability` (advisory; appears only when the preset is installed as a dependency and its GitHub release is reachable; PASS when attestation assets are present, WARN when the release lacks them; absent in all other circumstances). |
