...
| `doctor` | `doctor` | Structured diagnostic: 4-section checklist + `--json` output, exit 1 on BLOCKED. Configuration checks include: `release-it peer dependency` (installed version satisfies `peerDependencies.release-it` range; FAIL if absent or out of range), `release-it major version` (WARN when a newer upstream major exceeds the preset's validated peer range; silently skipped on network failure) and `Workspace dependency ranges` (workspace projects only: internal package dependency ranges checked against current workspace versions; advisory WARN on stale or unevaluable ranges, never FAIL; omitted on single-package repos). |
