...
For workspace projects, `doctor` also checks internal package dependency ranges. `workspace:` protocol references pass automatically; exact, `^`, `~`, `>=`, and supported `||` ranges are compared against the current workspace package version. Stale recognized ranges produce a WARN with a fix suggestion, while unsupported range syntax is skipped to avoid false warnings.
