...
**Version constraint (important):** `@release-it-plugins/workspaces` v5.0.3 declares peer `release-it ^17 || ^18 || ^19`, while this preset supports `^19 || ^20 || ^21`. The supported intersection is **release-it `^19`** — with release-it 20 or 21 installed, the plugin fails at module load with an unmet peer dependency error. Pin release-it to v19 when composing; prefer v21 standalone where its Node.js engine range is supported, or use v20 when a consumer must remain on Node 20.

This composition is exercised end-to-end in this repository's CI: `tests/e2e/workspaces-composition.test.ts` runs a real local release (`patch --ci`, git/github/npm publishing disabled) in a temp monorepo and asserts the plugin bumps workspace package versions and rewrites internal dependency ranges with the preset configuration loaded via `extends`. The release-it 20 and 21 incompatibilities are locked by tests, so the guidance above will be updated when the plugin widens its peer range.
